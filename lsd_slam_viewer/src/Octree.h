...
class Octree {
	...
	public:
	Octree(const Eigen::Vector3f& origin, const Eigen::Vector3f& halfDimension)
	...
	void getPointsInsideBox(const Eigen::Vector3f& bmin, const Eigen::Vector3f& bmax, std::vector<OctreePoint*>& results) {
		// If we're at a leaf node, just see if the current data point is inside
		// the query bounding box
		if(isLeafNode()) {
			if(data!=NULL) {
				const Eigen::Vector3f& p = data->getPosition();
			...
			for(int i=0; i<8; ++i) {
				...
				Eigen::Vector3f cmax = children[i]->origin + children[i]->halfDimension;
				Eigen::Vector3f cmin = children[i]->origin - children[i]->halfDimension;
